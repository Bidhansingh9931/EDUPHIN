...
class _AddAccountantPageState extends State<AddAccountantPage> {
  ...
  void _submit() async {
    ...
    try {
      ...
      if (_profileImage != null) {
        data['photo'] = base64Encode(await _profileImage!.readAsBytes());
      }
      ...
      if (response.statusCode == 200 && resBody['status'] == true) {
        ...
        throw Exception(resBody['message'] ?? 'Failed to add accountant');
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Add New Accountant")),
      body: SafeArea(
                ...
                child: Column(
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text("Save Accountant"),
                    ),
                    const SizedBox(height: 40),
                  ],
                ),
  ...
  Widget _buildFormGrid() {
    // Use 1 column for mobile, 2 for tablet/desktop
  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(
      ...
      children: children.map((c) => SizedBox(
        width: (MediaQuery.of(context).size.width - context.spacing * 2 - 16) / crossAxisCount,
        child: c,
      )).toList(),
    );
  }
