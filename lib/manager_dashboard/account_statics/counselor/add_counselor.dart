...
class _AddCounselorPageState extends State<AddCounselorPage> {
  ...
  // State for dropdowns, date pickers, and image
  String? _gender;
  ...
  void _addCounselor() async {
    ...
    try {
      ...
      if (_profileImage != null) {
        ...
        counselorData['photo'] = base64Encode(bytes);
      }

      final response = await ApiService.post('manager/users', counselorData);
      ...
      if (response.statusCode >= 200 && response.statusCode < 300 && responseData['status'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(responseData['message'] ?? 'Counselor added successfully!')),
        );
      ...
      floatingActionButton: Padding(
        ...
        child: SizedBox(
          ...
          child: FloatingActionButton.extended(
            heroTag: 'addCounselorBtn',
            onPressed: _isLoading ? null : _addCounselor,
            label: _isLoading
                ? const CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Colors.white))
                : Text("Add Counselor", style: theme.textTheme.labelLarge?.copyWith(color: theme.colorScheme.onPrimary)),
  ...
  Widget _buildNarrowLayout() {
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          _AddCounselorProfileBox(image: _profileImage, onPickImage: _pickImage),
...
class _AddCounselorProfileBox extends StatelessWidget {
  final File? image;
  final VoidCallback onPickImage;

  const _AddCounselorProfileBox({this.image, required this.onPickImage});
