...
class _AddCounselorPageState extends State<AddCounselorPage> {
  ...
  void _submit() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && resBody['status'] == true) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Counselor added!')));
        Navigator.pop(context, true);
      } else {
        throw Exception(resBody['message'] ?? 'Failed to add counselor');
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Add New Counselor")),
      body: SafeArea(
                ...
                child: Column(
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text("Save Counselor"),
                    ),
                    const SizedBox(height: 40),
                  ],
                ),
