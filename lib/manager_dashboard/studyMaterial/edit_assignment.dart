...
class _EditAssignmentPageState extends State<EditAssignmentPage> {
  ...
  void initState() {
    ...
    if (widget.assignment.dueDate != 'N/A' && widget.assignment.dueDate.isNotEmpty) {
      try {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          final initialClass = _classes.firstWhere((c) => c['name'] == widget.assignment.className, orElse: () => null);
          if (initialClass != null) {
            _selectedClassId = initialClass['id'];
            _sections = initialClass['sections'] ?? [];
            final initialSection = _sections.firstWhere((s) => s['section_name'] == widget.assignment.section, orElse: () => null);
            if (initialSection != null) {
              _selectedSectionId = initialSection['id'];
            }
          }

          final initialSubject = _subjects.firstWhere((s) => s['subject_name'] == widget.assignment.subject, orElse: () => null);
  ...
  Future<void> _updateAssignment() async {
    ...
    try {
      ...
      var request = http.MultipartRequest('POST', Uri.parse('${ApiService.baseUrl}/manager/study/assignments/${widget.assignment.id}'));
      ...
      if (response.statusCode == 200) {
         if(mounted){
           ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Assignment updated successfully!'), backgroundColor: Colors.green),
          );
        ...
        throw Exception(responseData['message'] ?? 'Failed to update assignment');
      }
    } catch (e) {
       if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: ElevatedButton(
        ...
        child: _isSaving ? const CircularProgressIndicator(valueColor: AlwaysStoppedAnimation(Colors.white)) : const Text('Update Assignment'),
      ),
    );
  }
}
